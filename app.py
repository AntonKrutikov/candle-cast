...
st.markdown(
    ...
    [class*="st-key-cc_card_chart"] .stPlotlyChart,
    [class*="st-key-cc_card_chart"] .js-plotly-plot {{
        min-height:600px;
    }}
    </style>
    """,
...
_INTERVAL_MINUTES = {"15m": 15, "30m": 30, "1h": 60, "4h": 240, "1d": 1440}


def _horizon_label(interval: str, n: int) -> str:
    total_min = n * _INTERVAL_MINUTES.get(interval, 60)
    if total_min < 60:
    ...
    d = total_min / 1440
    return f"{int(d)} days" if d == int(d) else f"{d:.1f} days"
...
with center:
    ...
    with st.container(key="cc_card_chart"):
        ...
        chart_slot.plotly_chart(
            render_chart(history, symbol), width="stretch", key="main_chart"
        )
        caption_slot.caption(
            f"Loaded {len(history)} candles · last: {history.index[-1]:%Y-%m-%d %H:%M} UTC"
        )

        summary_col, buttons_col = st.columns([4, 1], vertical_alignment="center")
        with summary_col:
            forecast_summary_slot = st.empty()
        with (
            buttons_col,
            st.container(
                horizontal=True,
            ...
            st.button(
                ...
                disabled=True,
                key="chart_download_btn",
                help="Download chart (coming soon)",
            )
            st.button(
                "",
                icon=":material/share:",
        ...
        if forecast_df is not None:
            ...
            for i in range(step, len(forecast_df) + step, step):
                chart_slot.plotly_chart(
                    ...
                    width="stretch",
                    key=f"main_chart_reveal_{i}",
                )
                time.sleep(0.03)

            if not forecast_df.empty:
                last_close = float(history["close"].iloc[-1])
                final_close = float(forecast_df["close"].iloc[-1])
                pct = (final_close - last_close) / last_close * 100
                if pct > 0.25:
                    icon, label, color, sign = (
                    ...
                    sign = "+" if pct >= 0 else ""
                forecast_summary_slot.markdown(
                    f":{color}[{icon} {label} outlook] · "
                    f":{color}[**{sign}{pct:.2f}%**] "
                    f":gray[projected over the next {_horizon_label(interval, len(forecast_df))}]"
                )
    ...
    legend_cols = st.columns(3, gap="medium")
    LEGEND = [
        (
            ":green[:material/candlestick_chart:]",
            "Historical candles",
            "Recent OHLCV market data before the forecast starts.",
        ),
        (
            ":primary[:material/auto_awesome:]",
            "AI forecast candles",
            "Predicted future candles generated by the AI model.",
        ),
        (
            ":primary[:material/unfold_more:]",
            "Forecast range",
            "Estimated high-low range across the prediction window.",
        ),
    ]
    for col, (icon, title, body) in zip(legend_cols, LEGEND):
        with col:
            st.markdown(f"{icon} **{title}**", text_alignment="center")
